...
// hecho
  draw() {
    ...
    ctx.fillRect(0, 0, canvas.width, canvas.height);
      ...
      // Dibujar cráteres
      const numCraters = 0;
      const angleStep = (Math.PI * 2) / numCraters;
      const craterRadius = this.radius * 0.1;

      for (let i = 0; i < numCraters; i++) {
        const angle = angleStep * i;
        const craterX =
          this.centerX + Math.cos(angle) * (this.radius - craterRadius * 1.5);
        const craterY =
          this.centerY + Math.sin(angle) * (this.radius - craterRadius * 1.5);
...
class Flower {
  ...
  draw() {
    ...
    var landscapeHeight = canvas.height * 0.1; // Ajusta este valor para cambiar la altura del paisaje
    ...
    const leafSize = this.stemHeight / 12; // Ajusta este valor para cambiar el tamaño de las hojas
    ctx.fillStyle = "#008000"; // Verde diferente para las hojas

    // Dibujar las hojas del tallo
    ctx.beginPath();
    ctx.ellipse(this.centerX - leafSize / 2, canvas.height - this.stemHeight / 2, leafSize * 2, leafSize / 2, 0, 0, Math.PI * 2);
    ctx.fill();
  ...
  generateLeaves() {
      const leafCount = 10; // Cantidad de hojas volando
      const minSize = 5;
      const maxSize = 10;
...
animate() {
  ...
  const animateStep = (timestamp) => {
    ...
    const amplitude = 15; // Ajusta este valor para cambiar la amplitud del movimiento
    ...
    if (progress < 1) {
      ...
      // Retrasar la eliminación de las hojas voladoras
      setTimeout(() => {
        this.leaves = []; // Limpiar las hojas voladoras
        this.draw();
      }, 1000); // Ajusta este valor para cambiar el tiempo de espera antes de eliminar las hojas
...
function animateTransition() {
  ...
  var animationInterval = setInterval(function() {
    // Borra el contenido del canvas en cada paso de la animación
    clearCanvas();

    // Calcula el radio actual de la transición (disminuye el radio del sol y aumenta el radio de la luna, y viceversa)
    var sunRadius = sun.radius - (sun.radius / steps) * currentStep;
    var moonRadius = (moon.radius / steps) * currentStep;
  ...
  }, 20); // Intervalo de tiempo entre cada paso de la animación
}
